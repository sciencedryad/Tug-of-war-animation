/** Applet for animation of tug-of-war of several plus and minus motors on a common cargo.
 *
...
public class TugOfWarAnimation extends Applet implements Runnable,ActionListener{

// animation parameters
	//final int L=800,H=480;				// width and height of canvas [pixel]
	int L,H;			// width and height of canvas [pixel], set in init()
	int sleepTime=100;
...
public void init(){
	// set up canvas, buffer image, graphics
		L=Math.max(800,this.getSize().width);	// width of canvas [pixel]
		H=390;	// height of canvas [pixel]
	...
	// set up control buttons and text fields
		...
		txt_message=new JTextArea(" Press the Run/Pause button to run the simulation.",2,50);txt_message.setFont(font);
			txt_message.setBackground(Color.white);
			messagePanel.add(txt_message);
		//cb_startHold=new JCheckBox("Run/Pause",true);cb_startHold.setFont(font);
		//	buttonPanel.add(cb_startHold);cb_startHold.addItemListener(new RunPauseListener());
		//	cb_startHold.setBackground(Color.blue);cb_startHold.setForeground(Color.white);
		...
		timeLab=new JLabel("     Time slowed down by factor of ");timeLab.setFont(font);buttonPanel.add(timeLab);
		...
		but_help=new Button("Help");but_help.setFont(font);
			buttonPanel.add(but_help);but_help.addActionListener(this);	
//		but_quit=new Button("Quit");but_quit.setFont(font);
//			buttonPanel.add(but_quit);but_quit.addActionListener(this);	
/*		but_paraInfo=new Button("Simulation info");but_paraInfo.setFont(font);
		...
		JPanel copyrightPanel=new JPanel();copyrightPanel.setBackground(panelColor);organizePanel.add(copyrightPanel);
			txt_copyright=new JLabel("<html>Parameter changes become valid only after pressing Enter in the input field.&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&copy; Melanie J.I. M&uuml;ller 2008, MPI-KG Potsdam</html>",JLabel.RIGHT);
		...
		// Tool Tips
			...
			//but_paraInfo.setToolTipText("Display information about simulation parameters");
			//but_info.setToolTipText("Display author information");
...
private final void drawMagnifiedCargo(Graphics gr,int Np,int Nm,int np,int nm){
		int kp,km;
	// delete background
//		gr.setColor(getBackground());	gr.fillRect(0,0,L,magFilamentHeight);
...
//		cargo.setCargoParas(Np,Nm,FsP,FdP,epsP,piP,vFP,vBP,FsM,FdM,epsM,piM,vFM,vBM);
	...
	// drawing
		...
		//displayMagnifiedCargoInfo(dbg);		// info about magnified cargo
		...
		//displaySimulationInfo(dbg);			// info about what is depicted
		//GraphicsUtil.drawLine(dbg,0,H,L,H,4);	// line at bottom of canvas
...
private final void displayCargoParas(Graphics gr){
	gr.setColor(getBackground());gr.fillRect(0,300-20,L,40);
	gr.setColor(Color.black);
	gr.drawString(
		"Np="+Np+", Nm="+Nm
		+", FsP="+FsP+", FdP="+FdP+", epsP="+epsP+", piP="+piP+", vFP="+vFP+", vBP="+vBP
		+", FsM="+FsM+", FdM="+FdM+", epsM="+epsM+", piM="+piM+", vFM="+vFM+", vBM="+vBM,10,300);
	gr.drawString("dt="+cargo.getdt()+", sleepTime="+sleepTime+", real time slowed down by "+timeFactor,10,320);
//		", dt = "+cargo.getdt()+", x = "+cargo.getPosition()+", t = "+cargo.getTime(),10,220);
}
...
public void run(){
	...
	while(tugAnimation!=null)
	{
		...
		// start/hold checkbox
			//while(cb_startHold.getState())try{Thread.sleep(100);} catch(InterruptedException e){}
			//while(cb_startHold.isSelected())try{Thread.sleep(100);} catch(InterruptedException e){}
...
//	if(stopExecution){tugAnimation.stop();}
	tugAnimation.start();
}
// public void stop(){tugAnimation=null;}
public void pleaseStop(){tugAnimation=null;}
...
public void actionPerformed(ActionEvent event){
	...
	if(event.getActionCommand()==but_startHold.getActionCommand()){ 			// run/pause button
		...
		//cb_startHold.setState(true);
		//cb_startHold.setSelected(true);
		run=false;
...
// 	else if(event.getActionCommand()==but_quit.getActionCommand()){ 			// Quit button
// 	    //try {getAppletContext().showDocument(new URL("http://www.google.de"));}
//   	    //try {getAppletContext().showDocument(new URL("file:///usr/www/people/mmueller/index.html"));}
//   	    try {getAppletContext().showDocument(new URL("http://www.mpikg.mpg.de/th/people/mmueller/"));}
//   	    //try {getAppletContext().showDocument(new URL("file:///index.html"));}
//      	 catch (Exception e) {e.printStackTrace();}
// 		txt_message.setForeground(Color.black);
// 		txt_message.setText(" Quit. ");
// 	}
	else if(event.getActionCommand()==but_help.getActionCommand()){ 			// Help button
//  	    try {getAppletContext().showDocument(new URL("http://www.mpikg.mpg.de/th/people/mmueller/"));}
  	    try {getAppletContext().showDocument(new URL(getCodeBase(),"Help.html"));}
     	 catch (Exception e) {e.printStackTrace();}
		txt_message.setForeground(Color.black);
		txt_message.setText(" Help. ");
	}
/*	else if(event.getActionCommand()==but_paraInfo.getActionCommand()){ 			// Simulation Info button
		JOptionPane.showMessageDialog(null,
			"Java applet for tug-of-war animation.\n"
			+"Filament length: 10 \u03BCm in total, periodic boundary conditions.\n"
			+"Simulation time is approximately real time if not slowed down by factor of 10 or 100,\n"
			+"           and if computer not busy otherwise.\n"
			+"Arrow: length and direction proportional to cargo velocity.\n"
			+"Caution: Simulation may be inaccurate for too large or too small parameter values\n"
			+"           (far from the kinesin values)."
